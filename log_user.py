...
def get_user_position(token, user_id):
    ...
    conn = mysql.connector.connect(**db_config)
    cursor = conn.cursor()
    cursor.execute("SELECT log_user.beacon_mac, beacons.type, time FROM log_user INNER JOIN beacons ON log_user.beacon_mac = beacons.mac_address WHERE log_user.userID = %s ORDER BY time DESC LIMIT 1", (user_id,))
    result = cursor.fetchone()
    cursor.close()
    conn.close()
